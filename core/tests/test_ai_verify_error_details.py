...
class AIVerifyErrorDetailsTests(TestCase):
    def setUp(self):
        self.subject = Subject.objects.create(name="Математика")
        self.exam_format = ExamFormat.objects.create(subject=self.subject, name="ЕГЭ", year=2026, is_active=True)
        self.task_type = TaskType.objects.create(
            exam_format=self.exam_format, number=20, name="Тип 20", max_points=2, is_extended_answer=True
        )
    ...
    def test_ai_failed_includes_upstream_status(self):
        ...
        payload = res.json()
        self.assertEqual(payload["error"], "ai_failed")
        self.assertEqual(payload.get("upstream_status"), 402)
