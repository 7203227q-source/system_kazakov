...
class StudentSolveAssignmentRendersNormalizedAIFeedbackTests(TestCase):
    def setUp(self):
        ...
        self.sub = Submission.objects.create(student=self.student, assignment=self.assignment, task=task, image_url=image)
        self.sub.primary_score = 1
        self.sub.ai_feedback = "Использовано: $$cos(fracpi2+2x)=-sin(2x)$$"
        self.sub.save()

    def test_page_contains_normalized_latex(self):
        ...
        self.assertContains(page, "\\cos(\\frac{\\pi}{2}+2x)=-\\sin(2x)")
